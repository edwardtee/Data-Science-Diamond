...
plt.scatter(y_test, y_test_pred, alpha=0.4, s=15, color="royalblue", label="Predicted vs Actual")
lo = min(y_test.min(), y_test_pred.min())
hi = max(y_test.max(), y_test_pred.max())
plt.plot([lo, hi], [lo, hi], color="red", linestyle="--", linewidth=2, label="Perfect Prediction (y = x)")
...
plt.scatter(y_test_pred, residuals, alpha=0.4, s=15, color="royalblue", label="Residuals")
...
sns.histplot(residuals, kde=True, color="royalblue", edgecolor="none", alpha=0.6)
plt.axvline(x=0, color="red", linestyle="--", linewidth=2, label="Zero Error")
plt.axvline(x=residuals.mean(), color="darkorange", linewidth=2, label=f"Mean = {residuals.mean():.2f}")
plt.xlabel("Residual")
plt.ylabel("Count")
